function fillMatrix(matrix, n, content) {
    for (let i = 0; i < n; i++) {
        // make an n element of array and each of them contain
...
function checkValid(matrix, row, column) {
    ...
    for (let i = 0; i < 8; i++) {
        if (matrix[row + directions[i][0]] != undefined) {
            if (
                matrix[row + directions[i][0]][column + directions[i][1]] !=
                undefined
            ) {
                if (
                    matrix[row + directions[i][0]][column + directions[i][1]] !=
    ...
    // se è viva
        ...
        if (counter > 6 || counter < 5) {
            // se le celle morte sono più di 6, quindi 7 o 8 oppure, sono minore di 5, quindi 4, 3, 2, 1 cambio il segno
...
function prettyPrint(matrix) {
    // trasformare ogni matrice passata in una stringa
...
function generateRandom(max) {
    // as the name suggests, this function generate a random number
    return Math.floor(Math.random() * max);
}

function generateCoordinates(matrix) {
    // this function generate an array of coordinates, eachone is
    ...
    for (let i = 0; i < generateRandom(dimension ** 2); ++i) {
        ...
        coordinates in randomCellsCoordinates
            ? null
            : randomCellsCoordinates.push(coordinates);
    }
...
function start() {
    // put in infinite loop the program
